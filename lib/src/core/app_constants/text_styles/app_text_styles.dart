import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
//import 'package:google_fonts/google_fonts.dart';

import '../colors/app_colors.dart';

class AppTextStyles {
  /// black
  static TextStyle black11 = TextStyle(
    fontSize: 11.0.sp,
    color: AppColors.black,
  );

  static TextStyle black11_26 = TextStyle(
    fontSize: 11.0.sp,
    color: AppColors.black26,
  );

  static TextStyle black14 = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.black,
  );

  static TextStyle black14Bold = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.black,
    fontWeight: FontWeight.bold,
  );

  static TextStyle black16 = TextStyle(
    fontSize: 16.0.sp,
    color: AppColors.black,
  );

  static TextStyle black16_26 = TextStyle(
    fontSize: 16.0.sp,
    color: AppColors.black26,
  );

  static TextStyle black16Bold = TextStyle(
    fontSize: 16.0.sp,
    color: AppColors.black,
    fontWeight: FontWeight.bold,
  );

  static TextStyle black18 = TextStyle(
    fontSize: 18.0.sp,
    color: AppColors.black,
  );

  static TextStyle black22 = TextStyle(
    fontSize: 22.0.sp,
    color: AppColors.black,
  );

  static TextStyle black18Bold = TextStyle(
    fontSize: 18.0.sp,
    color: AppColors.black,
    fontWeight: FontWeight.bold,
  );
  static TextStyle black24Bold = TextStyle(
    fontSize: 24.0.sp,
    color: AppColors.black,
    fontWeight: FontWeight.bold,
  );

  static TextStyle black30Bold = TextStyle(
    fontSize: 30.0.sp,
    color: AppColors.black,
    fontWeight: FontWeight.bold,
  );

  static TextStyle black32Bold = TextStyle(
    fontSize: 32.0.sp,
    color: AppColors.black,
    fontWeight: FontWeight.bold,
  );

  static TextStyle black34Bold = TextStyle(
    fontSize: 34.0.sp,
    color: AppColors.black,
    fontWeight: FontWeight.bold,
  );

  static TextStyle black36Bold = TextStyle(
    fontSize: 36.0.sp,
    color: AppColors.black,
    fontWeight: FontWeight.bold,
  );

  static TextStyle black44Bold = TextStyle(
    fontSize: 44.0.sp,
    color: AppColors.black,
    fontWeight: FontWeight.bold,
  );

  /// white
  static TextStyle white8 = TextStyle(
    fontSize: 8.0.sp,
    color: AppColors.white,
  );

  static TextStyle white10 = TextStyle(
    fontSize: 10.0.sp,
    color: AppColors.white,
  );

  static TextStyle white10Bold = TextStyle(
    fontSize: 10.0.sp,
    color: AppColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle white11 = TextStyle(
    fontSize: 11.0.sp,
    color: AppColors.white,
  );

  static TextStyle white12Bold = TextStyle(
    fontSize: 12.0.sp,
    color: AppColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle white14 = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.white,
  );

  static TextStyle white14Bold = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle white18Bold = TextStyle(
    fontSize: 18.0.sp,
    color: AppColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle white20 = TextStyle(
    fontSize: 20.0.sp,
    color: AppColors.white,
  );

  static TextStyle white28 = TextStyle(
    fontSize: 28.0.sp,
    color: AppColors.white,
  );

  static TextStyle white30 = TextStyle(
    fontSize: 30.0.sp,
    color: AppColors.white,
  );

  static TextStyle white32 = TextStyle(
    fontSize: 32.0.sp,
    color: AppColors.white,
  );

  static TextStyle white32Bold = TextStyle(
    fontSize: 32.0.sp,
    color: AppColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle white36Bold = TextStyle(
    fontSize: 36.0.sp,
    color: AppColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle white48Bold = TextStyle(
    fontSize: 48.0.sp,
    color: AppColors.white,
    fontWeight: FontWeight.bold,
  );

  /// Grey
  static TextStyle grey8 = TextStyle(
    fontSize: 8.0.sp,
    color: AppColors.grey,
  );

  static TextStyle grey10 = TextStyle(
    fontSize: 10.0.sp,
    color: AppColors.grey,
  );

  static TextStyle grey11 = TextStyle(
    fontSize: 11.0.sp,
    color: AppColors.grey,
    //color: Colors.black38,
  );

  static TextStyle grey12 = TextStyle(
    fontSize: 12.0.sp,
    color: AppColors.grey,
    //color: Colors.black38,
  );

  static TextStyle grey14 = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.grey,
  );

  static TextStyle grey16 = TextStyle(
    fontSize: 16.0.sp,
    color: AppColors.grey,
  );

  static TextStyle specGrey14 = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.grey,
    decoration: TextDecoration.lineThrough,
  );

  /// red
  static TextStyle red10 = TextStyle(
    fontSize: 10.0.sp,
    color: AppColors.red,
    //color: Colors.black38,
  );

  static TextStyle red11 = TextStyle(
    fontSize: 11.0.sp,
    color: AppColors.red,
    //color: Colors.black38,
  );
  static TextStyle red12 = TextStyle(
    fontSize: 12.0.sp,
    color: AppColors.red,
    //color: Colors.black38,
  );
  static TextStyle red14 = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.red,
  );

  static TextStyle red14Bold = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.red,
    fontWeight: FontWeight.bold,
  );

  static TextStyle red16 = TextStyle(
    fontSize: 16.0.sp,
    color: AppColors.red,
  );

  static TextStyle red16Bold = TextStyle(
    fontSize: 16.0.sp,
    color: AppColors.red,
    fontWeight: FontWeight.bold,
  );

  /// green
  static TextStyle green14 = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.green,
  );

  /// blue
  static TextStyle blue14 = TextStyle(
    fontSize: 14.0.sp,
    color: AppColors.blue,
  );

  static TextStyle blue16 = TextStyle(
    fontSize: 16.0.sp,
    color: AppColors.blue,
  );
}
